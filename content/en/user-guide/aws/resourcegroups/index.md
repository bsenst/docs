...
Start your LocalStack container using your preferred method. We will demonstrate how to create a Resource Group using the AWS CLI, and create an EventBridge rule to capture group lifecycle events and publish notifications. We will use tag-based query to create a resource group. However, you can also use CloudFormation stack-based queries to create a resource group.
...
You can now create a rule in EventBridge that captures events and sends them to your desired target service, using [`PutRule](https://docs.aws.amazon.com/eventbridge/latest/APIReference/API_PutRule.html) API and [`PutTargets`](https://docs.aws.amazon.com/eventbridge/latest/APIReference/API_PutTargets.html) API. Run the following command to create a rule in EventBridge:
...
Retrieve the Amazon Resource Name (ARN) of the new rule. You can now create a file, named `eventpattern.txt` with the following content:
...
Run the following command to retrieve the custom event pattern from the file:
...
You can now attach a target that can process the events to the EventBridge rule. Run the following command to attach an SNS topic to the EventBridge rule:

{{< command >}}
$ events put-targets \
    --rule CatchResourceGroupEventsForMyGroup \
    --targets Id=1,Arn=arn:aws:sns:us-east-1:000000000000:my-sns-topic
{{< /command >}}
